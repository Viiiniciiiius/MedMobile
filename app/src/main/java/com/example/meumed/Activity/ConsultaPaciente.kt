...
import com.example.medmobile.databinding.ActivityConsultaPacienteBinding
import com.google.firebase.auth.FirebaseAuth
import com.google.firebase.database.FirebaseDatabase
import java.text.SimpleDateFormat
import java.util.*

class ConsultaPaciente : AppCompatActivity() {
    private lateinit var binding: ActivityConsultaPacienteBinding
    private lateinit var adapter: AdapterSelecionarP
    private var selectedProfissional: SelecionarP? = null

    override fun onCreate(savedInstanceState: Bundle?) {
        binding = ActivityConsultaPacienteBinding.inflate(layoutInflater)
        super.onCreate(savedInstanceState)
        setContentView(binding.root)

        // Configuração do RecyclerView
        val recyclerViewSelecionarP: RecyclerView = binding.RecyclerViewSelecionarProC
        ...
        binding.bttListaC.setOnClickListener { transicaoListaC() }
        binding.bttVoltarC.setOnClickListener { finish() }
        binding.bttEnviarC.setOnClickListener { enviarSolicitacao(pacienteUid) }
    }
    ...
    private fun enviarSolicitacao(pacienteUid: String?) {
        val textoPac = binding.editTextC.text.toString().trim()
    ...
    private fun transicaoListaC() {
        val navegarListaC = Intent(this, ListaSolicitacoes::class.java)
        startActivity(navegarListaC)
    }

    private fun transicaoEnviarC() {
        val navegarEnviarC = Intent(this, ListaSolicitacoes::class.java)
        startActivity(navegarEnviarC)
        finish()
    }
}
