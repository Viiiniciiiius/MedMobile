...
import com.example.medmobile.databinding.ActivityDuvidasPacienteBinding
import com.google.firebase.auth.FirebaseAuth
import com.google.firebase.database.FirebaseDatabase
import java.text.SimpleDateFormat
import java.util.*

class DuvidasPaciente : AppCompatActivity() {
    private lateinit var binding: ActivityDuvidasPacienteBinding
    private lateinit var adapter: AdapterSelecionarP
    private var selectedProfissional: SelecionarP? = null

    override fun onCreate(savedInstanceState: Bundle?) {
        binding = ActivityDuvidasPacienteBinding.inflate(layoutInflater)
        super.onCreate(savedInstanceState)
        setContentView(binding.root)

        // Configuração do RecyclerView
        val recyclerViewSelecionarP: RecyclerView = binding.RecyclerViewSelecionarProDuv
        ...
        binding.bttListaDuv.setOnClickListener { transicaoListaDuv() }
        binding.bttVoltarDuv.setOnClickListener { finish() }
        binding.bttEnviarDuv.setOnClickListener { enviarSolicitacao(pacienteUid) }
    }
    ...
    private fun enviarSolicitacao(pacienteUid: String?) {
        val textoPac = binding.editTextDuv.text.toString().trim()
    ...
    private fun transicaoListaDuv() {
        val navegarListaDuv = Intent(this, ListaSolicitacoes::class.java)
        startActivity(navegarListaDuv)
    }

    private fun transicaoEnviarDuv() {
        val navegarEnviarDuv = Intent(this, ListaSolicitacoes::class.java)
        startActivity(navegarEnviarDuv)
        finish()
    }
}
