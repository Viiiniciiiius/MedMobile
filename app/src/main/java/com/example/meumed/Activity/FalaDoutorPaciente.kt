...
import com.example.medmobile.databinding.ActivityFalaDoutorPacienteBinding
import com.google.firebase.auth.FirebaseAuth
import com.google.firebase.database.FirebaseDatabase
import java.text.SimpleDateFormat
import java.util.*

class FalaDoutorPaciente : AppCompatActivity() {
    private lateinit var binding: ActivityFalaDoutorPacienteBinding
    private lateinit var adapter: AdapterSelecionarP
    private var selectedProfissional: SelecionarP? = null

    override fun onCreate(savedInstanceState: Bundle?) {
        binding = ActivityFalaDoutorPacienteBinding.inflate(layoutInflater)
        super.onCreate(savedInstanceState)
        setContentView(binding.root)

        // Configuração do RecyclerView
        val recyclerViewSelecionarP: RecyclerView = binding.RecyclerViewSelecionarProFalaDr
        ...
        binding.bttListaFalaDr.setOnClickListener { transicaoListaFalaDr() }
        binding.bttVoltarFalaDr.setOnClickListener { finish() }
        binding.bttEnviarFalaDr.setOnClickListener { enviarSolicitacao(pacienteUid) }
    }
    ...
    private fun enviarSolicitacao(pacienteUid: String?) {
        val textoPac = binding.editTextFalaDr.text.toString().trim()
    ...
    private fun transicaoListaFalaDr() {
        val navegarListaFalaDr = Intent(this, ListaSolicitacoes::class.java)
        startActivity(navegarListaFalaDr)
    }

    private fun transicaoEnviarFalaDr() {
        val navegarEnviarFalaDr = Intent(this, ListaSolicitacoes::class.java)
        startActivity(navegarEnviarFalaDr)
        finish()
    }
}
